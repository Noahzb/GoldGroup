$("#find-food").on("click", function (event) {
    ...
    $.ajax({
      ...
      $("#food-view").html(JSON.stringify(response.hits[0].recipe.label) + "<br>" + (response.hits[1].recipe.label)
       + "<br>"  + response.hits[2].recipe.label) + "<br>"  + (response.hits[3].recipe.label) 
